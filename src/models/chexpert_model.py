...
class VitCheXpert(torch.nn.Module):
    def __init__(self, image_model:torch.nn.Module, checkpoint_path:str='', freeze_backbone=True):
        super(VitCheXpert, self).__init__()
        self.freeze_backbone = freeze_backbone
        self.backbone = image_model
        if len(checkpoint_path)!=0:
            ## Reloading Weights
...
class MaskVLMCheXpert(torch.nn.Module):
    def __init__(self, image_model:torch.nn.Module, checkpoint_path:str='', freeze_backbone=True):
        super(MaskVLMCheXpert, self).__init__()
        self.freeze_backbone = freeze_backbone
        self.backbone = image_model
        if len(checkpoint_path)!=0:
            ## Reloading Weights
    ...
    def forward(self, input_batch):
        with torch.set_grad_enabled(not self.freeze_backbone):
            encoding = self.backbone(input_batch)[:, 0, :]
        output = self.decoder(encoding)
        return output    