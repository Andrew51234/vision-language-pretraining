#!/bin/bash
#SBATCH --job-name=CheXpert-convirt-no-flip-no-freeze
#SBATCH --output=logs/sbatch/CheXpert-convirt-no-flip-no-freeze-%A.out
#SBATCH --error=logs/sbatch/CheXpert-convirt-no-flip-no-freeze-%A.err
...
srun python src/finetune_chexpert.py +experiment=finetune/classification/convirt/no_flip/finetune_chexpert_convirt_no_flip_no_freeze