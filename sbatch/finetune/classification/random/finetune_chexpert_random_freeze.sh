#!/bin/bash
#SBATCH --job-name=CheXpert-random-freeze
#SBATCH --output=logs/sbatch/CheXpert-random-freeze-%A.out
#SBATCH --error=logs/sbatch/CheXpert-random-freeze-%A.err
...
srun python src/finetune_chexpert.py --config-name=finetune_chexpert_random +experiment=finetune/classification/random/finetune_chexpert_random_freeze