#!/bin/bash
#SBATCH --job-name=CheXpert-subset-01-random-freeze
#SBATCH --output=logs/sbatch/CheXpert-subset-01-random-freeze-%A.out
#SBATCH --error=logs/sbatch/CheXpert-subset-01-random-freeze-%A.err
...
srun python src/finetune_chexpert.py --config-name=finetune_chexpert_random +experiment=finetune/classification/random/finetune_chexpert_subset_01_random_freeze