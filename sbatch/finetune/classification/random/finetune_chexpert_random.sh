#!/bin/bash
#SBATCH --job-name=CheXpert-random
#SBATCH --output=logs/sbatch/CheXpert-random-%A.out
#SBATCH --error=logs/sbatch/CheXpert-random-%A.err
...
srun python src/finetune_chexpert.py --config-name=finetune_chexpert_random +experiment=finetune/classification/random/finetune_chexpert_random