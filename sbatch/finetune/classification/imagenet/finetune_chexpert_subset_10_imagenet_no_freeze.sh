#!/bin/bash
#SBATCH --job-name=CheXpert-subset-10-imagenet-no-freeze
#SBATCH --output=logs/sbatch/CheXpert-subset-10-imagenet-no-freeze-%A.out
#SBATCH --error=logs/sbatch/CheXpert-subset-10-imagenet-no-freeze-%A.err
...
srun python src/finetune_chexpert.py --config-name=finetune_chexpert_imagenet +experiment=finetune/classification/imagenet/finetune_chexpert_subset_10_imagenet_no_freeze