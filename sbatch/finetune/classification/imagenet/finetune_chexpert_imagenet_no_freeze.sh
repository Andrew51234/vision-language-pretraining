#!/bin/bash
#SBATCH --job-name=CheXpert-imagenet-no-freeze
#SBATCH --output=logs/sbatch/CheXpert-imagenet-no-freeze-%A.out
#SBATCH --error=logs/sbatch/CheXpert-imagenet-no-freeze-%A.err
...
srun python src/finetune_chexpert.py --config-name=finetune_chexpert_imagenet +experiment=finetune/classification/imagenet/finetune_chexpert_imagenet_no_freeze