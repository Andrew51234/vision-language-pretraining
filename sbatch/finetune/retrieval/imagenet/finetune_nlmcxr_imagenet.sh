#!/bin/bash
#SBATCH --job-name=NLMCXR-imagenet
#SBATCH --array=0-2  # indices 0, 1, and 2
#SBATCH --output=logs/sbatch/NLMCXR-imagenet-%A_%a.out
#SBATCH --error=logs/sbatch/NLMCXR-imagenet-%A_%a.err
...
experiments=(
  "finetune/retrieval/imagenet/finetune_nlmcxr_subset_01_imagenet"
  "finetune/retrieval/imagenet/finetune_nlmcxr_subset_10_imagenet"
  "finetune/retrieval/imagenet/finetune_nlmcxr_imagenet"
)
...
srun python src/finetune_nlmcxr.py --config-name=finetune_nlmcxr_imagenet +experiment=$experiment 